        ...
        return dbc.Card(
            [
                make_hideable(
                    dbc.CardHeader(
                        [
                            html.H3(self.title), # Usa self.title traduzido
                        ]
                    ),
                        ...
                        dbc.Row(
                            [
                                make_hideable(
                                    dbc.Col(
                                        [
                                            # Usa index_name que vem do explainer (já traduzido)
    ...
    def component_callbacks(self, app):
        ...
        def update_output_div(index, include_percentile, pos_label):
            if index is not None:
                # Markdown gerado dinamicamente pelo explainer
...
class FeatureDescriptionsComponent(ExplainerComponent):
    ...
    def to_html(self, state_dict=None, add_header=True):
        args = self.get_state_args(state_dict)
        # DataFrame já foi traduzido em explainer_methods.py
